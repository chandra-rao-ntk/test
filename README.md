#!/usr/bin/env python3
# Read-only PAN workflow (multi-VR reporting):
# - Panorama: objects+rules for multiple IPs, discover VRs per template (supports [ INT_VR EXT_VR ])
# - Firewalls: one session per FW; run FIB for ALL IPs across ALL VRs; show every match (VR+iface+zone)
# - Shows ALL FWs; star (★) rows whose DG has rules for the IP
# - Logs in ./logs/, stitched report saved there
...
# OPTIONAL overrides (mgmt_ip -> Panorama template name) if heuristic doesn't match
TEMPLATE_OVERRIDE = {
    # "10.232.240.153": "FBAS21SSFMW_Template",
}

# Device-group -> firewall mgmt IPs (used for ★ marker)
DG_TO_FIREWALLS = {
...
if ENABLE_NETMIKO_DEBUG:
    logging.basicConfig(filename=os.path.join(LOG_DIR, "netmiko_debug.log"),
                        level=logging.DEBUG,
...
def _write_log(fname: str, text: str):
    try:
        with open(os.path.join(LOG_DIR, fname), "w", encoding="utf-8") as f: f.write(text)
    except Exception:
        pass
...
def pano_template_vrs(conn):
    ...
    """
    conn.config_mode()
    out = conn.send_command('show | match "set template " | match " vsys vsys1 import network virtual-router "',
...
def build_fw_to_vrs_from_panorama(conn):
    ...
    for mgmt_ip, fw_name in FIREWALLS.items():
        ...
        fw_to_vrs[mgmt_ip] = tmpl_vrs.get(tmpl, [])
    return fw_to_vrs
...
def fw_worker_all_ips(fw_ip, fw_name, ip_list, pano_vrs_for_fw):
    ...
    One connection per FW:
      - use Panorama VR list; fall back to local discovery
...
def starred_fw_ips_from_dgs(rule_dgs):
    starred = set()
    for dg in rule_dgs:
        for ip in DG_TO_FIREWALLS.get(dg, []): starred.add(ip)
    return starred
...
def main():
    ...
    print(f"\n=== PAN Multi-IP Lookup (multi-VR reporting) ===")
    ...
    for ip in IPS_TO_CHECK:
        ...
        lines.append("Per-firewall routing & zone (ALL FWs; ★ = DG has matching rules; multiple rows if multiple VRs match):")
        ...
        for fw_ip, fw_name in sorted(FIREWALLS.items(), key=lambda kv: (kv[1], kv[0])):
            ...
            for row in rows:
                star = "★" if fw_ip in ctx["stars"] else " "
